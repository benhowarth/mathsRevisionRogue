...
$( document ).ready(function() {
	...
	dungeon=
	{
		...
		move:function(x,y){
			...
			dunLeft=dunLeft-((x)-(x*2*px2em($("#dungeonHolder").css("letterSpacing"))));
			dunTop=dunTop-((y)-(y*2*px2em($("#dungeonHolder").css("letterSpacing"))));
			console.log(dunLeft,dunTop);
		...
		question:function(){
			...
			dungeon.answer(prompt(question+"\n 0: "+answers[0]+"\n 1: "+answers[1]+"\n 2: "+answers[2]+"\n (type 'exit' to exit the question)"));
		},
	...
	function dungeonGen(dungeonObj)
	{
		...
		$("html").keydown(function(e){
			//up
			if(e.keyCode==87){dungeonObj.player.moveTo(dungeonObj.player.x,dungeonObj.player.y-1);if(dungeonObj.player.moved){dungeon.move(0,1);}}
			//down
			else if(e.keyCode==83){dungeonObj.player.moveTo(dungeonObj.player.x,dungeonObj.player.y+1);if(dungeonObj.player.moved){dungeon.move(0,-1);}}
			//left
			else if(e.keyCode==65){dungeonObj.player.moveTo(dungeonObj.player.x-1,dungeonObj.player.y);if(dungeonObj.player.moved){dungeon.move(1,0);}}
			//right
			else if(e.keyCode==68){dungeonObj.player.moveTo(dungeonObj.player.x+1,dungeonObj.player.y);if(dungeonObj.player.moved){dungeon.move(-1,0);}}
		});
